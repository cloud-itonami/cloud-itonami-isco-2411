(ns accounting.actor
  "AccountingActor — the ISCO-08 2411 independent accounting actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one accounting operation request
  ...
  The unconditional invariant: the AccountingAdvisor can never
  directly commit a record or dispatch a robot action the
  AccountingGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [accounting.advisor :as advisor]
            [accounting.governor :as governor]
            [accounting.store :as store]))

(defn build-graph
  "Build a compiled AccountingActor graph. `store` implements
  `accounting.store/Store`. `advisor` implements
  `accounting.advisor/Advisor` (defaults to `mock-advisor`).
